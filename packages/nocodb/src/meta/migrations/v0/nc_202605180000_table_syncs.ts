...
const up = async (knex: Knex) => {
  ...
  await knex.schema.createTable(MetaTable.TABLE_SYNCS, (table) => {
    ...
    table.dateTime('last_full_synced_at');
  ...
  await knex.schema.createTable(MetaTable.TABLE_SYNC_MAPPINGS, (table) => {
    ...
    // Reverse lookup — "which mappings depend on this share-link?". Used
    // by audit + proactive-invalidation flows that hold a uuid, not a view id.
